...
**확장진입 차단** (2026-09-11 실거래 487건 분석, `ENTRY_EXTENSION_GUARD`): 당일 고점
근접도(`day_range_pos`) ≥ 0.60, RSI ≥ 60, 전일 대비 거래대금비율 ≥ 0.75 중 하나라도
해당하면 점수와 무관하게 진입하지 않습니다. 세 팩터 모두 median-split 해보면 "이미
많이 올라온" 쪽이 그렇지 않은 쪽보다 오히려 실현수익률이 낮았습니다 — 초단기(중앙값
23분) 홀딩에서 모멘텀 팩터가 실제로는 상투를 잡고 있었다는 뜻입니다.

### 청산 (우선순위 순)

1. **손절** 매수가 대비 −1.5% (또는 ATR 기반 동적 손절가 중 먼저 닿는 쪽) → 전량 시장가
2. **트레일링 스탑** 1차 익절 이후, 고점 대비 −1.5% → 잔량 전량
3. **1차 익절** +3.0% → 보유 수량의 35%
4. **타임컷** 진입 후 60분간 ±1% 횡보 → 전량. 실현손익만 보면 순손실 2위 사유
   (-394,677원/150건)라 한때 꺼봤으나, 9일치 실데이터 리플레이로 반사실을 확인하니
   타임컷 없이 두면 그 포지션들이 손절/장마감 강제청산으로 더 크게 깨졌다
   (-30.44% → -40.81%, `tools/backtest.py --source db` 기준) — 원래대로 유지
5. **일괄 청산** 15:15 오버나잇 회피
6. **킬스위치** 당일 −3% → 전량 청산 + 당일 매매 중단 (한 번 켜지면 그날 안 꺼짐)

> 손절 −1.5%·1차익절 35%·트레일링 위주 구성은 2026-09-04~11 실거래(승률 42.2%,
> 기존 −2%/50% 구성 손익비 0.83)를 바탕으로 손익 비대칭을 완화한 값입니다. 트레일링
> 청산의 평균 실현수익률(+4.02%)이 1차익절(+3.12%)보다 높았던 데서 착안했습니다.
...
| 주문 거부 | 실시간 `00` 의 `919`(거부사유)를 감지해 즉시 자리를 풀고 알림. 단 `919`는 정상 통보에도 `"0"`(사유 없음)으로 실려 오므로 0이 아닌 값만 거부로 본다 |
...
| `919`의 `"0"`을 거부로 오판 | 모든 접수/체결 통보가 거부로 뒤집혀 체결이 기록되지 않고, `pending_exit`이 풀려 같은 청산이 초 단위로 재전송되며 매도가능수량 부족(800033)을 유발했다 |

...
```ini
KIWOOM_APP_KEY=여기에_발급받은_앱키_입력
KIWOOM_APP_SECRET=여기에_발급받은_시크릿_입력
KIWOOM_ENV=mock              # 실매매시 real 로 변경
DRY_RUN=true                 # true: 실제 주문은 나가지 않고 로그만 기록 (안전모드)
...
# .env 에 KIWOOM_APP_KEY / KIWOOM_APP_SECRET / TELEGRAM_* 입력
```
...
| 초당 호출 제한 | 토큰 버킷(기본 4 req/s) + 429 지수 백오프. 장중 시세는 웹소켓이라 REST 를 거의 안 씀 |
| 토큰 만료 | 만료시각 캐싱, 10분 전 갱신, 401 응답 시 자동 재발급 |
| 네트워크 단절 | 웹소켓 지수 백오프 재접속(최대 30초) + **구독 자동 복원** |
...
- **웹소켓 `LOGIN` / `PING` 은 PDF 본문에 명시되어 있지 않아** 키움 공식 파이썬 샘플의 규약(`{"trnm":"LOGIN","token":…}`, PING 원문 에코)을 따랐습니다.
  첫 접속에서 `WebSocket LOGIN 성공` 로그가 뜨는지 반드시 확인하세요.

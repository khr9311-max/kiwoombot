...
# [필수] KRX 데이터 계정 — 장 전 스크리닝용 일봉/시가총액 조회에 씁니다.
KRX_ID=KRX_아이디
KRX_PW=KRX_비밀번호
```

> **`KRX_ID` / `KRX_PW` 를 빼먹으면 봇이 매일 "감시 유니버스를 만들지 못했습니다" 만
> 보내고 한 종목도 매매하지 않습니다.** 스크리닝에 쓰는 `data.krx.co.kr` 이 로그인 없는
> 조회를 차단하기 때문입니다(응답 `400 LOGOUT`).
> [data.krx.co.kr](https://data.krx.co.kr) 에서 무료로 회원가입한 뒤 아이디/비밀번호를 넣으세요.
> (스크리닝을 건너뛰고 `FIXED_UNIVERSE` 로 특정 종목만 감시할 때는 필요 없습니다.)

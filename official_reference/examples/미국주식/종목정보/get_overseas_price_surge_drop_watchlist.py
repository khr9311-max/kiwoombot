# ---
# api_id: usa20932
# api_name: 미국주식 가격급등락(관심종목)
# category: 미국주식
# sub_category: 종목정보
# template: rest
# api_url: /api/us/stkinfo
# menu_path: 미국주식 > 종목정보 > 미국주식 가격급등락(관심종목)(usa20932)
# ---
...
def get_overseas_price_surge_drop_watchlist(
    stex_tp: str | None = '0',
    ...
    """
    미국주식 가격급등락(관심종목)[usa20932] API를 호출합니다.

    공통 클라이언트가 유효한 캐시 토큰을 사용하거나 필요 시 자동으로 발급합니다.

    Args:
        stex_tp: 거래소 구분 — 0:전체,1:NYSE,2:AMEX,3:NASDAQ
        stk_cd: 관심종목 — [{'stex_tp':'거래소구분','stk_cd':'종목코드'},...]
        flu_tp: 정렬 구분 — 1:급등(기본값),2:급락
        tm_tp: 기준 구분 — 1:분전(기본값),2:일전,3:지정일
        tm: 분전 값 — XX분전(최대30) or XX일전(최대30) or  tm_tp :3 입력 시, 기간(1:1일전,2:5일전,3:1개월전,4:3개월전,5:6개월전,6:연중,7:1년전,8:3년전,9:5년전)
    ...
    Example:
        >>> result = get_overseas_price_surge_drop_watchlist(
        ...     stex_tp='0',
    ...
    if stk_cd is not None:
        body["stk_cd"] = stk_cd  # 관심종목
    if flu_tp is not None:
        body["flu_tp"] = flu_tp  # 정렬 구분
    if tm_tp is not None:
        body["tm_tp"] = tm_tp  # 기준 구분
    if tm is not None:
        body["tm"] = tm  # 분전 값

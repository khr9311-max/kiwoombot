# ---
# api_id: usa20511
# api_name: 미국주식 기간별 등락률상위(ETF)
# category: 미국주식
# sub_category: 순위정보
# template: rest
# api_url: /api/us/rkinfo
# menu_path: 미국주식 > 순위정보 > 미국주식 기간별 등락률상위(ETF)(usa20511)
# ---
...
def get_overseas_change_rate_top_by_period_etf(
    stex_tp: str | None = '0',
    etf_cat1: str | None = '',
    etf_cat2: str | None = '',
    stk_cnd: str | None = '0',
    ...
    """
    미국주식 기간별 등락률상위(ETF)[usa20511] API를 호출합니다.

    공통 클라이언트가 유효한 캐시 토큰을 사용하거나 필요 시 자동으로 발급합니다.

    Args:
        stex_tp: 거래소구분 — 0:전체,1:NYSE,2:NASDAQ,3:AMEX
        etf_cat1: ETF코드1 — ETF 대카테고리코드 (ETF이고 대분류일 경우 사용), usa10105 cate1 속성 참고
        etf_cat2: ETF코드2 — ETF 중카테고리코드 (ETF이고 중분류일 경우 사용), usa10105 cate2 속성 참고
        stk_cnd: 종목조건 — 0:전체,1:증100%,2:증50%
        tm: n일전 구분 — 1:전일 5:5일 10:10일 30:30일,기본값:1
    ...
    Example:
        >>> result = get_overseas_change_rate_top_by_period_etf(
        ...     stex_tp='0',
        ...     etf_cat1='',
        ...     etf_cat2='',
        ...     stk_cnd='0',
    ...
    if etf_cat1 is not None:
        body["etf_cat1"] = etf_cat1  # ETF코드1
    if etf_cat2 is not None:
        body["etf_cat2"] = etf_cat2  # ETF코드2

...
# menu_path: 국내주식 > 종목정보 > 상하한가요청(ka10017)
# ---
...
def get_domestic_upper_lower_limit_stocks(
    mrkt_tp: str,
    updown_tp: str,
    sort_tp: str,
    stk_cnd: str,
    ...
    Args:
        ...
        sort_tp: 정렬구분 — 1:종목코드순, 2:연속횟수순(상위100개), 3:등락률순
        stk_cnd: 종목조건 — 0:전체조회,1:관리종목제외, 3:우선주제외, 4:우선주+관리종목제외, 5:증100제외, 6:증100만 보기, 7:증40만 보기, 8:증30만 보기, 9:증20만 보기, 10:우선주+관리종목+환기종목제외
        ...
        trde_gold_tp: 매매금구분 — 0:전체조회, 1:1천원미만, 2:1천원~2천원, 3:2천원~3천원, 4:5천원~1만원, 5:1만원이상, 8:1천원이상
    ...
    Example:
        >>> result = get_domestic_upper_lower_limit_stocks(
        ...     mrkt_tp='000',
        ...     updown_tp='1',
        ...     sort_tp='1',
        ...     stk_cnd='0',
    ...
    if not updown_tp:
        raise ValueError('updown_tp is required.')
    if not sort_tp:
    ...
    if not trde_gold_tp:
        raise ValueError('trde_gold_tp is required.')
    if not stex_tp:
